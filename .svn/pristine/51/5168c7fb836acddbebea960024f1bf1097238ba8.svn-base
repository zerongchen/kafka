...
SQL="use ${HIVE_DB};insert overwrite directory '${OUTPUTPATH}'
...
DB_TABLENAME=report_flow_d
DB_COLUMNS="R_STATTIME,FLOW_UP,FLOW_DN"
...
sqoop export --connect ${DB_URL} --username ${DB_USER} --password ${DB_PASSWD} --table ${DB_TABLENAME} --columns ${DB_COLUMNS} --export-dir ${EXPORT_DIR} --fields-terminated-by '\001' --input-null-string '\\N' --input-null-non-string '\\N'  -m 2>>${LOGFILE} 2>>${LOGFILE}
